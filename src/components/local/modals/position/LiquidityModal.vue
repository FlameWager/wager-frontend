...
export default defineComponent({
	...
	setup(props, context) {
		...
		const handleLogin = async () => {
			await flameWager.sdk.sync()
			flameWager.sdk.getPkh().then((pkh) => {
	...
	<Modal :show="show" width="500" new closable @onClose="$emit('onClose')">
		...
		<Flex direction="column" gap="32" :class="$style.base">
			...
			<div v-else-if="showHint.confirmationDelay" :class="$style.hint">
				...
				<a
					href="https://flameWager.notion.site/Transaction-confirmation-is-not-received-for-a-long-time-18f589e67d8943f9bf5627a066769c92"
					target="_blank"
