		...
		const overviewStats = computed(() => {
			if (activeTab.value === "bettors") {
				...
				return {
					...
					metric1Label: "Betting Volume (XTZ)",
					metric2: `${numberWithSymbol(totalWinnings.toFixed(2), ",")}`,
					metric2Label: "Total Winnings (XTZ)"
				}
			} else {
				...
				return {
					...
					metric1Label: "Combined TVL (XTZ)",
					metric2: `${numberWithSymbol(totalLP.toFixed(2), ",")}`,
					metric2Label: "Event LP Provided (XTZ)"
				}
			}
		})
			...
			<table v-else-if="activeTab === 'bettors'" :class="$style.table">
				...
				<tbody>
					...
					>
						...
						<td>{{ numberWithSymbol(parseFloat(user.totalBetsAmount || 0).toFixed(2), ",") }} XTZ</td>
						<td>
							<span :class="$style.winnings_text">
								+{{ numberWithSymbol(parseFloat(user.totalWinnings || 0).toFixed(2), ",") }} XTZ
							</span>
						</td>
						<td>
							<span 
			...
			<table v-else-if="activeTab === 'providers'" :class="$style.table">
				...
				<tbody>
					...
					>
						...
						<td>{{ numberWithSymbol(parseFloat(user.totalLiquidityProvided || 0).toFixed(2), ",") }} XTZ</td>
						<td>{{ numberWithSymbol(parseFloat(user.totalPoolDeposits || 0).toFixed(2), ",") }} XTZ</td>
						<td>
