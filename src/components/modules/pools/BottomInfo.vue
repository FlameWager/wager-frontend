...
<template>
	<Flex direction="column" gap="24">
		<PoolsGuidesModal :show="showPoolsGuidesModal" @onClose="showPoolsGuidesModal = false" />

		<Flex align="center" justify="between">
			<Flex direction="column" gap="8">
				<Flex align="center" gap="4">
					<Text size="13" weight="600" color="secondary"> Contracts have not yet been audited </Text>
					<Icon name="warning" size="12" color="yellow" />
				</Flex>

				<Text size="12" weight="600" color="tertiary"> Smart Contract&nbsp; •&nbsp; Liquidity Pools </Text>
			</Flex>

			<Flex gap="8">
				<Button @click="showPoolsGuidesModal = true" type="secondary" size="small">
				...
				<Dropdown side="top" disable-autofocus>
					...
					<template #dropdown>
						...
						<DropdownItem @click="router.push('/pools/list')"> <Icon name="table" size="16" /> Pools </DropdownItem>
						<DropdownItem @click="router.push('/pools/positions')"> <Icon name="table" size="16" /> My Positions </DropdownItem>
					</template>
				</Dropdown>
			</Flex>
		</Flex>

		<svg width="100%" height="5" fill="none" xmlns="http://www.w3.org/2000/svg">
			<defs>
				<pattern id="bg" patternUnits="userSpaceOnUse" width="85" height="24">
					<path
		...
		<Flex align="center" :class="$style.labels">
			<Text size="13" weight="500" color="support" :class="$style.label"> Minimum&nbsp;<span>0.01 XTZ</span> </Text>
			<Text size="13" weight="500" color="support" :class="$style.label">
				Version&nbsp;<span>{{ pool.version }}</span>
			</Text>
		</Flex>

		<Text size="12" weight="500" color="support" height="16" :class="$style.hint">
			Before interacting with the liquidity pool, read
			<a href="/docs/pools-overview" target="_blank">our guide</a>. Pools do not guarantee a stable yield and accurate rates of
			return.
		</Text>
	</Flex>
</template>
